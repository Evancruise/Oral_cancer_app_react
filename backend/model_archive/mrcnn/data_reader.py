import numpy as np
from mrcnn.dataset_helper import Dataset
from mrcnn.dataset_helper import data_generator
from mrcnn.unet_config import Config
...
def add_hillshade_channel(batch_hills, amt, images, config):
    ...
    print('hills shape:', np.array(hills).shape)
    # x = np.stack((images, hills), axis =1)
    img_arr = np.empty([amt, config.N_CHANNELS, 768, 1024])  # shape <class 'tuple'>: (600, 6, 256, 256)
    img_arr[:, 0:3, :, :] = images
    img_arr[:, 3:4, :, :] = hills
    return img_arr
...
def get_patches_dataset(dataset, config, shuffleOn = True, amt=10000, verbose = False, norm = True, imageNames = [], min_truth_sum = 0):
    ...
    x = images #add_hillshade_channel(batch_hills, amt, images, config)
    y = masks
